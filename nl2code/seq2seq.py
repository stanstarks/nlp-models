...
class Encoder(nn.Module):
    def __init__(self, padding_idx=0, rnn_class='lstm',
        ...
        # TODO: use load_embedding and try fix embedding
    ...
    def forward(self, xes, x_lens):
        bsz = len(xes)

        # embed input tokens
        xes = F.dropout(xes, p=self.dropout, training=self.training)
...
class Decoder(nn.Module):
    """
    output is moved out from this part
    """
    def __init__(self, padding_idx=0, rnn_class='lstm',
                 emb_size=128, hidden_size=128, num_layers=2, dropout=0.1,
        ...
        self.rule_embedding_W = nn.Embedding(config.rule_num, config.rule_embed_dim,
    ...
    def forward(self, xs, hidden, encoder_output, attn_mask=None):
        """
        inputs:
        xs: decode step and feed in target
        hidden:
